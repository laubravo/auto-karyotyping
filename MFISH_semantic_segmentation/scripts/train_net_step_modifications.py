...
import detectron_pytorch.utils.misc as misc_utils
from detectron_pytorch.core.config import (
    cfg, cfg_from_file, cfg_from_list, assert_and_infer_cfg)
from detectron_pytorch.datasets.roidb import combined_roidb_for_training
from detectron_pytorch.roi_data.loader import (
    RoiDataLoader, MinibatchSampler, BatchSampler, collate_minibatch)
from detectron_pytorch.modeling.model_builder import Generalized_RCNN
from detectron_pytorch.utils.detectron_weight_helper import (
...
from detectron_pytorch.utils.training_stats import TrainingStats
...
def main():
    ...
    elif args.dataset == "MFISH_train":
        cfg.TRAIN.DATASETS = ('MFISH_train',)
        cfg.MODEL.NUM_CLASSES = 25
    else:
    ...
    if args.load_detectron:  #TODO resume for detectron weights (load sgd momentum values)
        ...
        # restore layers to coco sizes
        maskRCNN.Box_Outs.cls_score = nn.Linear(2048, 81, bias=True)
        maskRCNN.Box_Outs.bbox_pred = nn.Linear(2048, 324, bias=True)
        maskRCNN.Mask_Outs.classify = nn.Conv2d(256,81,1)

        load_detectron_weight(maskRCNN, args.load_detectron)
        # change layers to MFISH sizes
        maskRCNN.Box_Outs.cls_score = nn.Linear(2048, 25, bias=True)
        maskRCNN.Box_Outs.bbox_pred = nn.Linear(2048, 100, bias=True)
        maskRCNN.Mask_Outs.classify = nn.Conv2d(256, 25, 1)        
