   ...
   # Freeze model, train only the last FC layer for the transfered task
    if args.fine_tuning:
        ...
        # INCEPTION
            ...
            # Last layer
            num_ftrs = model.fc.in_features
            model.mfc = nn.Linear(num_ftrs, num_classes)
            parameters = model.fc.parameters()
            # print(model)
            # quit()
        else:
...
def validate(val_loader, model, criterion):
    ...
    file = open(args.saveDir+'/test.txt',"a")
...
def save_checkpoint(state, is_best, filename):
    full_filename = filename + '/checkpoint.pth.tar'
    torch.save(state, full_filename)
    if is_best:
        new_filename = filename + '/model_best.pth.tar'
